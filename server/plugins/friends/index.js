...
exports.register = (server, options, next) => {
  ...
  const friendsArr = [/*eslint-disable max-len */
    {
      ...
      img: "//goo.gl/I9utJF",
      size: 8,
      github: "https://github.com/electrode-io/electrode-boilerplate-universal-react-node"
    }, {
      name: "React",
      img: "//goo.gl/xwbqlB",
      size: 8,
  ...
  const getContributorsPromises = friendsArr.map((friend) => {/*eslint-disable camelcase */
    ...
    return githubGetContributors(githubInfo).then((response) => {
      friend.friends = response.map(({login, avatar_url, html_url}) => {
        return {name: login, img: avatar_url, profile: html_url};
      });
      return friend;
    });
